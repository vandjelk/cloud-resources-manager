...
func kcpNetworkLocalWait(ctx context.Context, st composed.State) (error, context.Context) {
	...
	if meta.IsStatusConditionTrue(state.LocalNetwork().Status.Conditions, cloudcontrolv1beta1.ConditionTypeReady) {
		return nil, ctx
	}

	if meta.IsStatusConditionTrue(state.LocalNetwork().Status.Conditions, cloudcontrolv1beta1.ConditionTypeError) {
		changed := false
	...
	logger.Info("Waiting KCP local network ready state")
