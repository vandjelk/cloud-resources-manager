...
func (r *reconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil {
		return ctrl.Result{}, fmt.Errorf("error creating AwsCertificate state: %w", err)
	}
	action := r.newAction()

	return composed.Handling().
		WithMetrics("awscertificate", util.RequestObjToString(request)).
		WithNoLog().
...
func (r *reconciler) newAction() composed.Action {
	return composed.ComposeActions(
		"crAwsCertificateMain",
		feature.LoadFeatureContextFromObj(&cloudresourcesv1beta1.AwsCertificate{}),
